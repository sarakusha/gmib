...
const SearchDialog: React.FC<Props> = ({ open, close }) => {
	...
	useEffect(() => {
		...
		if (!open) {
			addressRef.current &&
				(defaultValues.current.address = addressRef.current.value);
			mibTypeRef.current &&
				(defaultValues.current.type = mibTypeRef.current.value);
		}
	}, [open, kind]);
				...
				>
					...
					>
						...
						<Select
							...
							input={
								<Input name="mibtype" id="mibtype" inputRef={mibTypeRef} />
							}
							fullWidth
						>
				...
				>
					<List sx={{ minWidth: '40ch' }}>
						{detected.map((info) => {
							const mib =
								info.type > 0
									? window.nibus.findMibByType(info.type, info.version)
									: undefined;
							return (
