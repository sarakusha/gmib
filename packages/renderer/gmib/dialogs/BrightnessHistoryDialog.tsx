...
const getSensors = async (): Promise<Sensors[]> => {
	...
	if (isRemoteSession) {
		const now = Date.now();
		const signature = await window.identify.generateSignature(
			'GET',
			sensorsUrl,
			now,
		);
...
const getBands = (
	...
	const sunriseEnd =
		suntimes.sunriseEnd && new Date(suntimes.sunriseEnd).getTime();
	const goldenHourEnd =
		suntimes.goldenHourEnd && new Date(suntimes.goldenHourEnd).getTime();
	const goldenHour =
		suntimes.goldenHour && new Date(suntimes.goldenHour).getTime();
	const sunsetStart =
		suntimes.sunsetStart && new Date(suntimes.sunsetStart).getTime();
	...
	const solarNoon =
		suntimes.solarNoon && new Date(suntimes.solarNoon).getTime();
...
const BrightnessHistoryDialog: React.FC<Props> = ({
	open = false,
	onClose = noop,
}) => {
	...
	const lastIlluminance = useSelector((state) =>
		selectLastWithAddress(state, 'illuminance'),
	);
	useEffect(() => {
		if (!open) return;
		Promise.all([window.nibus.getBrightnessHistory(), getSensors()]).then(
			([history, sensors]) => {
				setOptions((opts) => {
					const series = Object.entries(
						...
						([address, values]) =>
							({
								...
								color: '#FF8000',
								shadow: true,
								data: values.map(({ time, illuminance }) => [
									time,
									illuminance,
								]),
					...
					const data = history.map(
						({
							...
							timestamp: number;
							brightness: number;
						}) => [timestamp - (timestamp % 1000), brightness],
					);
					...
					if (isValidLocation) {
						...
						const [yPlotBands, yPlotLines] = getBands(
							latitude,
							longitude,
							yesterday,
						);
	...
	React.useEffect(() => {
		...
		setOptions((opts) => {
			...
			const brightnessSeries = opts.series?.find(
				({ id }) => id === 'brightness',
			);
				...
				{suntimes && (
					<Box sx={{ display: 'flex', justifyContent: 'space-evenly' }}>
						<Grid>
							...
							<Typography>
								{suntimes.sunriseEnd.toLocaleTimeString()}
							</Typography>
						...
						<Grid>
							...
							<Typography>
								{suntimes.goldenHour.toLocaleTimeString()}
							</Typography>
