interface Window {
    ...
    readonly nibus: { createDevice: { (parent: DeviceId, address: string, mib: string): void; (parent: DeviceId, address: string, type: number, version?: number | undefined): void; }; findMibByType: any; readonly reloadDevices: any; readonly setLogLevel: any; readonly ping: (address: string) => Promise<[-1, undefined] | [number, Version]>; readonly setDeviceValue: (deviceId: DeviceId) => (name: string, value: import("../common/helpers").ValueType) => Promise<void>; readonly sendConfig: _.DebouncedFunc<(state: Record<string, unknown>) => void>; readonly reloadDevice: (deviceId: DeviceId) => Promise<void>; readonly releaseDevice: (deviceId: DeviceId) => void; readonly writeToStorage: (deviceId: DeviceId) => Promise<boolean>; readonly findDevices: (options: import("../common/helpers").FinderOptions) => Promise<void>; readonly cancelSearch: () => Promise<void>; readonly telemetry: ((id: DeviceId) => import("../common/helpers").NibusTelemetry) & _.MemoizedFunction; readonly getBrightnessHistory: any; readonly mibTypes: { value: string | number | symbol; name: any; }[]; readonly flash: (id: DeviceId, kind: any, filename: string | undefined, moduleSelect?: number | undefined) => Promise<void>; readonly addPlayerListener: (path: string) => void; readonly removePlayerListener: (path: string) => void; readonly hasPlayerListener: (path: string) => boolean; };
    readonly config: { readonly get: <Key extends keyof import("../common/helpers").LocalConfig>(key: Key) => Promise<import("../common/helpers").LocalConfig[Key]>; readonly set: <Key extends keyof import("../common/helpers").LocalConfig>(key: Key, value: import("../common/helpers").LocalConfig[Key]) => Promise<void>; };
