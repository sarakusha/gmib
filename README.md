...
номеров версии (например 4.2.0). Программа автоматически установится в папку, с `KNOWNFOLDERID`,
равным `FOLDERID_UserProgramFiles`, по пути `%LocalAppData%\Programs\gmib`. Данные и настройки
программы хранятся в папке `%APPDATA%\gmib`. Удаление программы осуществляется стандартными
средствами Windows через _Установка и удаление программ_ в _Параметрах системы_.
