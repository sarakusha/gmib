...
общий mDNS-транспорт на основе `bonjour-service`. В Windows отдельный responder привязывается к
`0.0.0.0` для каждого активного IPv4-интерфейса, поэтому системный Bonjour Service может работать
одновременно с GMIB, а его отсутствие или остановка не отключает обнаружение.
