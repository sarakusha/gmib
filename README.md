...
исходящий UDP-порт 5353. Если другой GMIB отвечает по HTTP, но не найден через mDNS, приложение
покажет предупреждение с кнопкой копирования готовых команд PowerShell. Команды следует выполнить
от имени администратора, после чего полностью перезапустить GMIB.

Публикация локального GMIB, поиск компьютеров для меню и выбор главного экземпляра используют один
общий mDNS-транспорт. В Windows при наличии Bonjour Service приложение использует системный Bonjour:
это исключает конкуренцию нескольких сокетов за UDP-порт 5353 и автоматически учитывает все активные
сетевые интерфейсы. Если Bonjour Service не установлен, используется встроенный responder, который
привязывается к каждому активному IPv4-интерфейсу.
