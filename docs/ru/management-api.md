...
задаётся в миллисекундах от `100` до `3600000` и применяется и к получению headers, и к чтению body.
Длительные операции, например загрузка обновления, могут использовать увеличенное значение. После
узнаваемого ответа middleware `{"identifier":"..."}` helper один раз выполняет новый SRP login и
безопасно повторяет запрос: middleware отклоняет его до обработчика. Другие ответы `401`, включая
plugin/application errors, автоматически не повторяются, чтобы action не выполнился дважды.
