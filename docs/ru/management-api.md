...
Все перечисленные маршруты уже существуют, кроме явно отмеченного административного маршрута S1.
Ответы и тела — JSON, если в таблице не указано иное. Мутации обычных ресурсов требуют действующей
лицензии; чтение разрешено до активации, но по-прежнему требует авторизации. Пустой успешный ответ
может иметь статус `200` или `204` в зависимости от маршрута.
...
| Смена пароля, добавлено в S1 | `PUT /api/manage/v1/auth/password`                                                                                                                         | `{salt, verifier}`; helper принимает новый пароль локально и вычисляет эти параметры сам. Маршрут доступен до активации лицензии.                                         |
...
Пока отсутствуют и будут добавляться отдельными этапами:

- noninteractive install/update/enable/disable/uninstall plugin — S3;
- проверенный контракт настройки plugin — S4;
- GET/PATCH разрешенных параметров конфигурации, включая параметры и флаг автояркости — S7.
...
```bash
printf '%s\n' '{"name":"Резервный плеер","width":1920,"height":1080}' |
...
side effects; полноценное сравнение desired/actual для Ansible будет добавлено вместе с ресурсными
командами.
