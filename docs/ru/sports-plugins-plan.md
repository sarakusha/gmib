...
Дата: 2026-09-14. Статус: предложение к реализации; перечисленные расширения API пока не реализованы.

## Результат первой версии

gmib запускается с главным окном, скрытым в трей, восстанавливает выбранный вывод на LED-экран.
Оператор открывает локальную PWA, выбирает команды и составы, ведёт хоккейный матч.
...
1. **Зависимости и обнаружение.** Добавить `dependencies` с диапазонами версий и
   ...
   Для хоккея использовать стабильный `sportId: icehockey`, как в Matchpad.
...
2. **Межплагинный сервис.** `context.services.provide/require` с версионированными контрактами
   ...
   Изменение справочников выполняется через backend/WebUI roster. Хоккей получает снимки через
...
Порядок старта: миграции → roster → hockey → восстановление состояния → готовность страницы → LED.
