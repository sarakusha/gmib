...
`POST /api/screen` не сохраняет `addresses` и `brightness`. Helper создаст экран, проверит ответ и
при необходимости выполнит завершающий `PUT` с прочитанными полями. Если этот `PUT` не удался,
ошибка содержит `createdId` и `operations:["create"]`: повторите команду с этим ID или тем же exact
name, чтобы не создавать дубль.
