...
UTF-8, а консольный шрифт — за наличие русских глифов. Перед выводом запроса provisioning ждёт
завершения udev и повторно применяет шрифт после позднего переключения консоли DRM/i915.
...
USB-подключение NovaStar Taurus определяется как RNDIS-сетевая карта. Kiosk запрашивает на ней адрес
по DHCP, а при отсутствии DHCP использует IPv4 link-local. Выданные RNDIS шлюз и DNS игнорируются,
чтобы Taurus не перехватывал основной доступ в интернет.
...
`vpnProfile` — base64 от прежнего raw OpenVPN-профиля или tar-архива Pritunl. Проверенный raw-профиль
устройство локально упаковывает в однофайловый tar перед импортом; безопасный готовый tar Pritunl
передаётся клиенту без изменений. Для соединения без шифрования укажите
`tlsConnect: "unencrypted"` и не передавайте поля PSK. Выключенный мониторинг —
