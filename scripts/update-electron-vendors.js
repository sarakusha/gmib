...
function updateVendors() {
  ...
  const [nodeMajorVersion] = electronRelease.node.split('.');
  const [major, minor] = electronRelease.v8.split('.');
  const chromeMajorVersion = `${major}${minor}`; // electronRelease.v8.split('.')[0] + electronRelease.v8.split('.')[1];
